...
class Drivetrain : public frc2::SubsystemBase {
 ...
 private:
  frc::Spark m_leftMotor{0};	// equivalent to m_leftMotor = frc::Spark(0)
  ...
  frc::Encoder m_rightEncoder{6, 7};
	...
	// Equivalent to
	// m_drive = frc::DifferentialDrive(lambda1, lambda2);
	// where each lambda is [&](double output) { motor.Set(output); }
